...
madlib_path = os.path.join(currPath, "MadCityLabs\\Madlib.dll")
...
class MCLNanoDrive(object):  # thedevice
    ...
    #    if x is not None:
    #        assert -1 <= x <= self.cal_X, f"x value {x} is out of range (0, {self.cal_X})"
    #        self.set_pos_ax(x, 1)
    #    if y is not None:
    #        assert -1 <= y <= self.cal_Y

    #        self.set_pos_ax(y, 2)

    def set_pos(self, x=None, y=None, z=None):
         if x is not None:
             x = max(0, min(x, self.cal_X))
             self.set_pos_ax(x, 1)
         if y is not None:
             y = max(0, min(y, self.cal_Y))
             self.set_pos_ax(y, 2)
    ...
    #     assert 1 <= axis <= self.num_axes, f"Invalid axis number {axis}. Valid axis numbers are 1 to {self.num_axes}."
    #     assert -50 <= pos <= self.cal[axis]-50, f"Position {pos} is outside the valid range of 0 to {self.cal[axis]} for axis {axis}."

    #     self.handle_err(madlib.MCL_SingleWriteN(c_double(pos), axis, self._handle))

    def set_pos_ax(self, pos, axis):
        print(f"Setting position {pos} for axis {axis}")
        if self.debug:
            print("set_pos_ax ", pos, axis)
        assert 1 <= axis <= self.num_axes, f"Invalid axis number {axis}. Valid axis numbers are 1 to {self.num_axes}."
        assert -1 <= pos <= self.cal[
            axis], f"Position {pos} is outside the valid range of -50 to {self.cal[axis]} for axis {axis}."
        # add a print statement here
        print(f"Position {pos} is valid for axis {axis}")
