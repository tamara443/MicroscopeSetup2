...
def load_data():
    ...
    wavelengths = df['Wavelengths'].values.astype(float) # ensure float

    # Convert wavelengths (nm) to energy (eV)
    energies = 1240.0 / wavelengths  # eV

    # Sort energies in ascending order for consistency
    ...
    corrected_df = df.sub(dark_count, axis=0)

    # Apply Jacobian transformation to the intensity
    # Compute 1240 / E^2 (note: E is already sorted)
    jacobian_factor = 1240.0 / (energies ** 2)

    # Reorder corrected_df according to the sorted energies
    corrected_df = corrected_df.iloc[sort_idx].reset_index(drop=True)

    # Apply the Jacobian correction
    corrected_df = corrected_df.multiply(jacobian_factor, axis=0)

    return energies, corrected_df
...
def fit_gaussian(energies, intensities):
    # Initial guess for the parameters: [amplitude, mean, standard deviation, baseline offset]
    initial_guess = [max(intensities) - min(intensities), energies[np.argmax(intensities)], 0.05, min(intensities)]
    try:
        popt, _ = curve_fit(gaussian_with_baseline, energies, intensities, p0=initial_guess)
        a, b, c, d = popt
...
def save_data_to_file(save_folder,filename, energies, intensities, fitted_curve=None, a=None, b=None, fwhm=None, d=None):
...
def find_peak_within_range(energies, intensities, energy_min, energy_max, prominence=0.1, width=5):
    ...
    if len(filtered_energies) == 0 or len(filtered_intensities) == 0:
        return None, None
    ...
    peak_energy = filtered_energies[peak_index]
    peak_intensity = filtered_intensities[peak_index]

    return peak_energy, peak_intensity

def PL_analysis(save_folder, df, energies, x_step_size, y_step_size, x_check, y_check, energy_min, energy_max, prominence, width):
    ...
    for column in df:
        ...
        if x == target_x and y == target_y:
            ...
            intensity_without_spikes = fixer(target_column.values, m=3, threshold=3.5, peak_regions=[(energy_min, energy_max)])
            ...
            plt.plot(energies, intensity_without_spikes, label='Data')
            ...
            peak_energy, peak_intensity = find_peak_within_range(energies, intensity_without_spikes,
                                                                     energy_min, energy_max, prominence, width)

            if peak_energy is not None:
                # Fit Gaussian over the entire wavelength range
                a, b, fwhm, d = fit_gaussian(energies, intensity_without_spikes)
                if a is not None and (energy_min <= b <= energy_max):
                    fitted_curve = gaussian_with_baseline(energies, a, b, fwhm / (2 * np.sqrt(2 * np.log(2))), d)
                    plt.plot(energies, fitted_curve, label='Gaussian Fit with Baseline')
                    plt.legend()
                    ...
                    save_data_to_file(save_folder, filename, energies, intensity_without_spikes, fitted_curve, a, b,
                                      fwhm, d)
                else:
                    # Save the data to a file without fit
                    filename = f"PL_Spectrum_X_{x_check}_Y_{y_check}_no_fit.txt"
                    save_data_to_file(save_folder, filename, energies, intensity_without_spikes)
            else:
                # Save the data to a file without fit
                filename = f"PL_Spectrum_X_{x_check}_Y_{y_check}_no_fit.txt"
                save_data_to_file(save_folder, filename, energies, intensity_without_spikes)
...
def get_wavelength_range():
    while True:
        try:
            ...
            peak_min = 1240.0 / float(input("Maximum Wavelength (in nm): "))
            peak_max = 1240.0 / float(input("Minimum Wavelength (in nm): "))
...
def main():
    ...
    energies, corrected_df = load_data()
    print("Select a folder to save the results.")
    save_folder = select_directory()
    energy_min, energy_max = get_wavelength_range()
    ...
    while True:
        ...
        PL_analysis(save_folder, corrected_df, energies, x_step_size, y_step_size, x_check, y_check, energy_min, energy_max,
                    prominence, width)
